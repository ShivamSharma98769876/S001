                ...
                if stop_loss_trigger_count < MAX_STOP_LOSS_TRIGGER:
                    ...
                    if new_strike and not adjusted_for_14_points and not adjusted_for_28_points and not profit_booking_occurred:
                        ...
                        if new_order_id:
                            ...
                            # Calculate loss from the previous trade
                            loss_taken += (current_total_premium - initial_total_premium)
                            # loss_taken += initial_total_premium - current_total_premium
                            # loss_taken += previous_loss
                            logging.info(f"Call strike replaced. loss_taken : {loss_taken:.3f} | Total loss taken: {loss_taken:.3f}")
                ...
                if stop_loss_trigger_count < MAX_STOP_LOSS_TRIGGER:
                    ...
                    if new_strike and not adjusted_for_14_points and not adjusted_for_28_points and not profit_booking_occurred:
                        ...
                        if new_order_id:
                            ...
                            # Calculate loss from the previous trade
                            loss_taken += (current_total_premium - initial_total_premium)
                            logging.info(f"Put strike replaced. loss_taken : {loss_taken:.3f} | Total loss taken: {loss_taken:.3f}")
    ...
    while True:
        try:
            ...
            if is_expiry_within_2_days(current_expiry):
                logging.info("Current expiry is within 2 days, finding next week's Tuesday expiry")
                next_expiry = get_next_week_expiry(options)
                options = [o for o in options if o['expiry'] == next_expiry]
                logging.info(f"Next week's Tuesday expiry: {next_expiry}")
            else:
