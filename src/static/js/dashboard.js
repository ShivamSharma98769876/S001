...
async function updateStatus() {
    ...
    try {
        const response = await throttledFetch('/api/dashboard/daily-stats', {}, 2000);
        if (!response) return; // Request was throttled
        if (!response.ok) {
            throw new Error(`HTTP ${response.status}: ${response.statusText}`);
        }
        ...
        const dailyStats = data.dailyStats || data.loss_protection || {};
        const dailyLoss = dailyStats.lossUsed || dailyStats.daily_loss || 0;
        const lossLimit = dailyStats.lossLimit || dailyStats.daily_loss_limit || 5000;
        const lossPercentage = dailyStats.lossPercentage || (lossLimit > 0 ? (dailyLoss / lossLimit * 100) : 0);
        ...
        if (lossUsedEl) lossUsedEl.textContent = formatCurrency(dailyLoss);
        if (lossLimitEl) lossLimitEl.textContent = formatCurrency(lossLimit);
        
        // Update progress bar
        const progressEl = document.getElementById('lossProgress');
        if (progressEl) {
            progressEl.style.width = Math.min(lossPercentage, 100) + '%';
        }
...
async function syncOrdersFromZerodha() {
    ...
    try {
        const response = await fetch('/api/dashboard/sync-orders', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({})
        });
        ...
        if (data.success || data.status === 'success') {
            const synced = data.synced_count || data.trades_created || 0;
            const skipped = data.skipped_count || 0;
            const errors = data.error_count || 0;
            
            let message = `✅ Order sync completed! Created ${synced} trade records`;
            if (skipped > 0) {
            ...
            addNotification(`Failed to sync orders: ${errorMsg}`, 'danger');
        }
    } catch (error) {
...
async function updateTrades() {
    ...
    try {
        ...
        // Debug logging
        console.log('[Trade History] Date filter:', dateFilter ? dateFilter.value : 'none');
        console.log('[Trade History] Trades received:', trades.length);
        if (trades.length > 0) {
            console.log('[Trade History] First trade sample:', trades[0]);
        }
        ...
        if (data.summary) {
            console.log('[Trade History] Summary received:', data.summary);
            updateTradeSummary(data.summary);
        } else {
            console.warn('[Trade History] No summary data in response');
            // Calculate summary from trades if backend didn't provide it
            if (trades.length > 0) {
                const calculatedSummary = {
                    totalTrades: trades.length,
                    totalProfit: trades.filter(t => (t.pnl || t.realized_pnl || 0) > 0).reduce((sum, t) => sum + (t.pnl || t.realized_pnl || 0), 0),
                    totalLoss: Math.abs(trades.filter(t => (t.pnl || t.realized_pnl || 0) < 0).reduce((sum, t) => sum + (t.pnl || t.realized_pnl || 0), 0)),
                    netPnl: trades.reduce((sum, t) => sum + (t.pnl || t.realized_pnl || 0), 0),
                    winRate: trades.length > 0 ? (trades.filter(t => (t.pnl || t.realized_pnl || 0) > 0).length / trades.length * 100) : 0
                };
                console.log('[Trade History] Calculated summary from trades:', calculatedSummary);
                updateTradeSummary(calculatedSummary);
            }
        }
        ...
        tbody.innerHTML = trades.map(trade => {
            // Backend sends: symbol, entryTime, exitTime, entryPrice, exitPrice, quantity, pnl, type
            // Support both camelCase (from backend) and snake_case (legacy) formats
            const symbol = trade.symbol || trade.trading_symbol || '-';
            const entryTimeStr = trade.entryTime || trade.entry_time || '';
            const exitTimeStr = trade.exitTime || trade.exit_time || '';
            const entryPrice = trade.entryPrice || trade.entry_price || 0;
            const exitPrice = trade.exitPrice || trade.exit_price || 0;
            const quantity = trade.quantity || 0;
            const pnl = trade.pnl || trade.realized_pnl || 0;
            const transactionType = trade.type || trade.transaction_type || (quantity > 0 ? 'BUY' : 'SELL');
            const isOpen = trade.is_open === true;
            
            // Get transaction type (BUY or SELL)
            ...
            // Format times in IST
            const entryTime = entryTimeStr ? formatDateTimeIST(entryTimeStr) : '-';
            const exitTime = isOpen ? '-' : (exitTimeStr ? formatDateTimeIST(exitTimeStr) : '-');
            const exitPriceDisplay = isOpen ? '-' : `₹${exitPrice.toFixed(2)}`;
...
function updateTradeSummary(summary) {
    ...
    // Backend sends camelCase: totalTrades, totalProfit, totalLoss, netPnl, winRate
    // Support both camelCase (from backend) and snake_case (legacy) formats
    const totalTrades = summary.totalTrades || summary.total_trades || 0;
    const totalProfit = summary.totalProfit || summary.total_profit || 0;
    const totalLoss = summary.totalLoss || summary.total_loss || 0;
    const netPnl = summary.netPnl || summary.total_pnl || summary.net_pnl || 0;
    ...
    // Win Rate - use value from backend (already calculated as profit_trades / total_trades * 100)
    const winRateValue = parseFloat(winRate) || 0;
    const winRateEl = document.getElementById('winRate');
    winRateEl.textContent = winRateValue.toFixed(1) + '%';
    // Green if win rate is >= 50%, red if < 50%
    winRateEl.style.color = winRateValue >= 50 ? '#10b981' : '#ef4444';
}
...
async function updateDailyStats() {
    ...
    try {
        const response = await throttledFetch('/api/dashboard/daily-stats', {}, 5000);
        if (!response) return; // Request was throttled
        if (!response.ok) {
            throw new Error(`HTTP ${response.status}: ${response.statusText}`);
        }
...
async function loadPnlCalendarData() {
    try {
        ...
        try {
            const apiUrl = `/api/dashboard/pnl-calendar?start_date=${startStr}&end_date=${endStr}`;
...
async function updateCumulativePnl(fullUpdate = false) {
    ...
    try {
        ...
        const url = needsFullUpdate ? '/api/dashboard/cumulative-pnl' : '/api/dashboard/cumulative-pnl?day_only=true';
        ...
        if (needsFullUpdate) {
            ...
            if (cumulativePnlCache.data) {
                ...
                try {
                    const fullResponse = await fetch('/api/dashboard/cumulative-pnl');
                    if (fullResponse.ok) {
